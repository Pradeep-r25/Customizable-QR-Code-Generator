...
import io
import webcolors  # Install this library using `pip install webcolors`
import os

# Initialize or read the serial number from a file
...
def gen_qr():
    ...
    # Create the QR code as an image and adjust its color
    qr_image = io.BytesIO()
    qr.png(qr_image, scale=10, module_color=color_fg_hex, background=color_bg_hex)
    qr_image.seek(0)  # Move the cursor to the beginning of the file

    # Use the in-memory image (BytesIO) to open the image
    img = Image.open(qr_image)  # Use in-memory image
    
    # Resize and convert image for display
    img = img.convert("RGB")
    img.thumbnail((200, 200))  # Resize to fit within the window
    
    img_tk = ImageTk.PhotoImage(img)
    
    # Update the label with the generated QR code
    l4.config(image=img_tk)
    l4.image = img_tk  # Keep a reference to avoid garbage collection
...
def save():
    ...
    # Get the current serial number for the file
    sno = get_serial_number()

    # Create a safe filename based on the serial number
    file_name = f"qr_{sno:03d}"  # Format as 'qr_001', 'qr_002', etc.
    ...
    # Increment serial number and update the file
    sno += 1
    update_serial_number(sno)

    con.set('')  # Clear the input field
...
# Variable
con = StringVar()
color_fg_var = StringVar(value="black")  # Default color is black
color_bg_var = StringVar(value="white")  # Default color is white
format_var = StringVar(value="PNG")  # Default format
...
formats = ['PNG', 'SVG', 'PDF']  # Available formats
...
format_menu.current(0)  # Set default to 'PNG'

# This label will display the generated QR code
